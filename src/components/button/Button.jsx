...
function Button({ name, variant = 'primary', icon = true, href, onClick, external = false }) {
  ...
  return (
    <button className={`${baseStyles} ${variants[variant] ?? variants.primary}`} onClick={onClick}>
      {content}
    </button>
  )
}
